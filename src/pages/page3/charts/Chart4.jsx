...
export default function Chart4({ data, labelAnio1, labelAnio2 }) {
  const top10Data = useMemo(() => {
    ...
    // Ordenamos de mayor a menor cantidad de casos
  ...
  return (
    <div className="flex flex-col gap-8 bg-white p-6 rounded-lg shadow-sm border border-gray-100">
      <h2 className="text-xl font-bold text-gray-800">Incapacidades por Rango de Edad</h2>
          ...
          <tbody>
            {top10Data.map((row, index) => (
              <tr key={index} className="border-b hover:bg-gray-50 transition-colors">
                <td className="p-3 text-gray-800 font-medium truncate max-w-xs" title={row.rangoEdad}>{row.rangoEdad}</td>
