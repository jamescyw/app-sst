...
export default function Chart1({ data, labelAnio1, labelAnio2 }) {
  ...
  const chartOptions = useMemo(() => {
    // ECharts dibuja el eje Y de abajo hacia arriba. Invertimos para que el Top 1 quede arriba.
    ...
    return {
      ...
      yAxis: {
        type: 'category',
        data: reversedData.map(d => d.cargo), // Usamos la propiedad cargo
        axisLabel: {
          width: 150, 
          overflow: 'truncate',
          fontSize: 11
          ...
          <tbody>
            {top10Data.map((row, index) => (
              <tr key={index} className="border-b hover:bg-gray-50 transition-colors">
                <td className="p-3 text-gray-800 font-medium truncate max-w-xs" title={row.cargo}>
                  {row.cargo}
                </td>
