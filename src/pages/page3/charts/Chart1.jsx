...
export default function Chart1({ data, labelAnio1, labelAnio2 }) {
  ...
  const chartOptions = useMemo(() => {
    ...
    return {
      ...
      yAxis: {
        type: 'category',
        data: reversedData.map(d => d.cargo),
        axisLabel: {
  ...
  return (
    <div className="p-6 rounded-xl border border-[#503629]  flex flex-col gap-8">
      <h2 className="text-xl font-bold text-center text-[#503629]">
        Top 10 Cargos con Mayor Incapacidad
      </h2>
          ...
          <tbody className="divide-y divide-gray-100">
            {top10Data.map((row, index) => (
              <tr key={index} className="hover:bg-[#faf9f8] transition-colors">
                <td className="p-3 text-gray-700 font-medium truncate max-w-[200px]" title={row.cargo}>
                  {row.cargo}
                </td>
