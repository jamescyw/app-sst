...
export default function OsteomuscularModule() {
  ...
  const clearSelectFilters = () => {
    setFilterCiudad(""); setFilterCargo(""); setFilterArea("");
    setFilterDepartamento(""); setFilterDireccion(""); setCurrentPage(1);
  };

  const handleValidarLider = async () => {
    if (!liderCC.trim()) return setLiderError("Ingrese la cédula del líder.");
    setValidandoLider(true); setLiderError("");
    try {
  ...
  const stats = computeOsteoStats(reportes);
    ...
    <div className="p-6 bg-white rounded-lg shadow-sm border border-gray-100 flex flex-col gap-6 ">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Gestión de casos y reportes osteomusculares (SVE)</p>
        </div>
        <button 
